...
The primary workflow for a TFS orgna's migration. Reads the **MDR-4ADO-AllProjects.xlsx** spreadsheet ("GAMS-Repos-App-Folder level" worksheet) and processes every row:
...
./Push-ToGitHub.ps1 -ConfigPath ./config/migration-config.json `
    -RepoPath ./output/app-a-repo -GitHubOrg "GhOrgName" -GitHubRepo "app-a-repo"
```
...
| Setting | Description |
|---|---|
| `adoServerUrl` | Your ADO 2022 base URL (e.g. `https://ado.domain.com`) |
...
| `github.enterpriseUrl` | GitHub Enterprise URL (e.g. `https://github.domain.com`) |
...
DOMAIN\jsmith,John Smith,jsmith@domain.com
DOMAIN\jdoe,Jane Doe,jdoe@domain.com
```
